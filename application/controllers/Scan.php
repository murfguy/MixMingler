	...
	public function users() {
		...
		foreach ($streamersData as $streamer) {
			...
			if ($minglerData == null) {
				...
				$news_data[] = $this->news->getNewsInsertQueryDataArray($minglerData->mixer_id, "{username} was first synced to MixMingler!", 'mingler');

				// So we also want to note for their activity feed that they started streaming something new.
				$news_data[] = $this->news->getNewsInsertQueryDataArray($minglerData->mixer_id, "{username} started streaming {typeId:".$streamer['type']['id']."}.", 'type', $streamer['type']['id']);
			} else {
		...
		if (count($lastStarted_data) > 0) {
			$this->db->update_batch('mixer_users', $lastStarted_data, 'mixer_id');
		}

		// Add we now batch insert all the tasty news items we collected.
		$this->db->insert_batch('timeline_events', $news_data);

		// And now we batch UPDATE every streamer we just collected data for.
		$this->db->update_batch('mixer_users', $update_data, 'mixer_id');
